...
const outbound = { id: 'm1', subtaskId: 's1', direction: 'outbound', status: 'sent', fromAddr: 'persona_ams@reply.inqi.io', toAddr: 'sales@acme.io', body: 'Price + availability?', createdAt: '2026-06-28T12:00:00Z' };
const inbound = { id: 'm2', subtaskId: 's1', direction: 'inbound', status: 'received', fromAddr: 'sales@acme.io', toAddr: 'persona_ams@reply.inqi.io', body: 'In stock, 540 EUR.', createdAt: '2026-06-28T13:00:00Z' };
...
test('thread renders: header (persona/hub/route/status) + direction-aligned bubbles', async ({ page }) => {
  ...
  await loadBoard(page, board([{ id: 's1', epicId: 'e1', subjectProviderName: 'Acme Bikes', wave: 1, status: 'replied', personaId: 'persona_amsterdam' }]));
...
test('a live message.received appends a new inbound bubble in order (no refresh)', async ({ page }) => {
  ...
  await page.goto('/#/admin/t/s1');
  await loadBoard(page, board([{ id: 's1', epicId: 'e1', subjectProviderName: 'Acme Bikes', wave: 1, status: 'contacted', personaId: 'persona_amsterdam' }]));
  ...
  await page.route('**/api/comms/thread/*', (r: Route) => r.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([outbound, inbound]) }));
  await dispatch(page, { id: '90', type: 'message.received', inquiryId: 'i1', epicId: 'e1', subtaskId: 's1', at: 'now', data: { from: 'sales@acme.io' } });
  await expect(page.getByTestId('bubble')).toHaveCount(2);
...
test('an un-contacted subtask shows the queued/empty state', async ({ page }) => {
  await seed(page);
  ...
  await loadBoard(page, board([{ id: 's2', epicId: 'e1', subjectProviderName: 'Queued Co', wave: 1, status: 'pending' }]));
...
test('Back routes to the board', async ({ page }) => {
  ...
  await page.route('**/api/comms/thread/*', (r: Route) => r.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([outbound]) }));
  await page.goto('/#/admin/t/s1');
  await loadBoard(page, board([{ id: 's1', epicId: 'e1', subjectProviderName: 'Acme Bikes', wave: 1, status: 'contacted' }]));
  await page.getByTestId('thread-back').click();
  await expect(page).toHaveURL(/#\/admin\/i\/i1/);
});
