...
test('a 401 from a data call redirects to Sign in', async ({ page }) => {
  ...
  await page.route('**/api/reports', (r: Route) => r.fulfill({ status: 401, contentType: 'application/json', body: JSON.stringify({ error: { code: 'AUTH_TOKEN_EXPIRED', message: 'expired' } }) }));
