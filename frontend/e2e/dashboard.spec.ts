...
test('lists reports, updates a row on a live transition, and routes by stage', async ({ page }) => {
  await page.addInitScript((s) => localStorage.setItem('inqi.session', s), SESSION);
  const reports = [{ id: 'inq11111aaaa', rawRequest: 'a used road bike, Amsterdam', state: 'OUTREACH', customerEmail: 'c@x.io', createdAt: new Date().toISOString() }];
  await page.route('**/api/reports', (r) => r.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(reports) }));
  ...
  await page.evaluate(() => {
    (window as unknown as { __inqiDispatch: (a: unknown) => void }).__inqiDispatch({
      type: 'realtime/event',
      event: { id: '98', type: 'report.transitioned', reportId: 'inq11111aaaa', at: 'now', data: { from: 'OUTREACH', to: 'OUTREACH', stage: 'Partially ready', qualifiedCount: 2 } },
    });
  });
  await expect(page.getByTestId('report-status')).toHaveText('Partially ready');

  // Live: dispatch report.transitioned → row badge becomes Ready (real reducer path).
  await page.evaluate(() => {
    (window as unknown as { __inqiDispatch: (a: unknown) => void }).__inqiDispatch({
      type: 'realtime/event',
      event: { id: '99', type: 'report.transitioned', reportId: 'inq11111aaaa', at: 'now', data: { to: 'REPORT_DELIVERED', stage: 'Ready' } },
    });
  });
