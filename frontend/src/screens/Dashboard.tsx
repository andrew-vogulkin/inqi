...
import { Stage, stageForInquiry, stageIndex, isFailedState, isLiveStage, stageBadge, routeForStage, STAGE_ORDER } from '../conventions/stages';
...
/** FE-03 — authenticated home: own inquiries (6-stage pipeline) + credit balance (prototype-matched). */
export function Dashboard() {
  ...
  const visible = order.filter((id) => {
    const i = inquiries.byId[id];
    return i && stageForInquiry({ state: i.state, stage: i.stage, qualifiedCount: i.qualifiedCount }) !== Stage.Draft;
  });
  ...
  return (
    <div>
      <header style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: space[4], marginBottom: space[6] }}>
        <div>
          <h1 style={{ fontSize: fontSize.h1, fontWeight: fontWeight.semibold, letterSpacing: '-.02em', margin: `0 0 ${space[1]}px` }}>Your inquiries</h1>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: space[2] }}>
          ...
          <a href={hrefFor({ route: Route.NewInquiry })}
      ...
      {inquiries.status === AsyncStatus.Ready && visible.length === 0 && <EmptyDash />}

      {visible.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: space[2] }}>
          {visible.map((id) => <InquiryRow key={id} inquiry={inquiries.byId[id]} />)}
        </div>
      )}
    </div>
  );
...
function InquiryRow({ inquiry }: { inquiry: InquiryDto }) {
  const stage = stageForInquiry({ state: inquiry.state, stage: inquiry.stage, qualifiedCount: inquiry.qualifiedCount });
  const idx = stageIndex(stage);
  const failed = isFailedState(inquiry.state);
  const badge = stageBadge({ stage, state: inquiry.state });
  ...
  return (
    <button onClick={go} data-testid="inquiry-row"
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: space[2], marginBottom: 9 }}>
          <span style={{ fontSize: fontSize.lg, fontWeight: fontWeight.medium, letterSpacing: '-.01em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{inquiry.rawRequest.slice(0, 80)}</span>
          <MonoRef muted>#{inquiry.id.slice(0, 8)}</MonoRef>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
          ...
          <span data-testid="inquiry-status" style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: fontSize.sm, fontWeight: fontWeight.medium, color: fg }}>
          ...
          <span style={{ fontSize: fontSize.xs, color: color.subtle }}>started {new Date(inquiry.createdAt).toLocaleDateString()}</span>
        </div>
      </div>
...
function EmptyDash() {
  return (
    <div style={{ border: `1px dashed ${color.lineStrong}`, borderRadius: radius.lg, padding: `56px ${space[8]}px`, textAlign: 'center', background: color.surface }}>
      ...
      <a href={hrefFor({ route: Route.NewInquiry })} style={{ display: 'inline-flex', height: 40, padding: `0 ${space[5]}px`, alignItems: 'center', borderRadius: radius.md, background: color.ink, color: color.onSolid, fontSize: fontSize.md, fontWeight: fontWeight.medium, textDecoration: 'none' }}>Start your first inquiry</a>
    </div>
  );
}
