...
const msg = (over: Partial<ThreadMessageDto>): ThreadMessageDto => ({ id: 'm', subtaskId: 's1', direction: MessageDirection.Outbound, status: MessageStatus.Sent, fromAddr: 'p@reply.io', toAddr: 'sales@acme.io', body: 'hi', createdAt: '2026-06-28T12:00:00Z', ...over });
const loaded = () => threadReducer(initialThreadState, { type: ActionType.ThreadLoaded, subtaskId: 's1', messages: [msg({ id: 'a' })] });
...
describe('threadReducer', () => {
  ...
  it('is idempotent by event id and scoped to the subtask', () => {
    let s = loaded();
    ...
    const before = s;
    const after = threadReducer(s, { type: ActionType.EventReceived, event: ev({ id: '7', subtaskId: 'other', data: { id: 'z', body: 'x' } }) });
  ...
  it('switching subtask resets the seen set', () => {
    let s = loaded();
    s = threadReducer(s, { type: ActionType.EventReceived, event: ev({ id: '9', data: { id: 'r1', body: 'x' } }) });
    s = threadReducer(s, { type: ActionType.ThreadLoaded, subtaskId: 's2', messages: [] });
    expect(s.subtaskId).toBe('s2');
