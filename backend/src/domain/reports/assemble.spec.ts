...
import { assembleOptions, FindingLike } from './reports.service';

const opt = (subtaskId: string, subjectProvider: string, price: number): FindingLike =>
  ({ kind: FindingKind.Option, subtaskId, data: { subjectProvider, price, currency: 'EUR' } });
const bg = (subtaskId: string, qualityScore: number): FindingLike =>
  ({ kind: FindingKind.SubjectProviderBackground, subtaskId, data: { qualityScore, rating: 4.5 } });

describe('assembleOptions', () => {
  it('merges background into the option by subtask and exposes qualityScore', () => {
