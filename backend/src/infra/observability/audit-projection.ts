...
export const project = {
  denial(inq: { id: string; denyReason: string | null; updatedAt: Date }): AuditEntry {
    return { type: AuditEntryType.Denial, at: iso(inq.updatedAt), actor: AuditActor.System, reason: inq.denyReason ?? undefined, inquiryId: inq.id };
  },
  complianceMessage(m: { reviewStatus: string; riskScore: number | null; riskTags: string[]; createdAt: Date; inquiryId: string }): AuditEntry {
    return { type: AuditEntryType.ComplianceBlock, at: iso(m.createdAt), actor: AuditActor.Compliance, inquiryId: m.inquiryId, data: { surface: ComplianceKind.Email, riskScore: m.riskScore, riskTags: m.riskTags } };
  },
  complianceQuestionnaire(q: { reviewStatus: string; riskScore: number | null; createdAt: Date; inquiryId: string }): AuditEntry {
    return { type: AuditEntryType.ComplianceBlock, at: iso(q.createdAt), actor: AuditActor.Compliance, inquiryId: q.inquiryId, data: { surface: ComplianceKind.Questionnaire, riskScore: q.riskScore } };
  },
  ...
  transition(e: { inquiryId: string; createdAt: Date; data: unknown }): AuditEntry {
    ...
    return { type: AuditEntryType.Transition, at: iso(e.createdAt), actor: AuditActor.Engine, inquiryId: e.inquiryId, data: { from: d.from, to: d.to, event: d.event } };
  },
  operator(a: { actor: string; action: string; targetType: string; targetId: string; reason: string | null; createdAt: Date; data: unknown }): AuditEntry {
    return {
      ...
      inquiryId: a.targetType === AuditTargetType.Inquiry ? a.targetId : undefined,
